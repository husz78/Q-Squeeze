...
In all experiments, pruning was restricted to the repeated decoder stack. The input token embedding matrix, final normalization layer, and final vocabulary projection (\texttt{lm\_head}) were kept intact. For Wanda, we zeroed individual weights in every \texttt{nn.Linear} module inside each decoder block, including both standard attention projections, Qwen 3.5 linear-attention projections, and MLP projections. For width pruning, we physically sliced standard self-attention query/key/value/output projections and MLP intermediate dimensions; Qwen 3.5 linear-attention modules were left unchanged. ShortGPT operates at a coarser granularity by removing selected decoder blocks while leaving the embedding and output layers unchanged.
...
\toprule
\textbf{Model / Pruning} & \textbf{Sparsity} & \textbf{MMLU Acc.} & \textbf{MMLU SE} & \textbf{GSM8K Acc.} & \textbf{GSM8K SE} \\
\midrule
...
\bibitem[Muralidharan et al.(2024)]{muralidharan2024compact} Muralidharan, S., et al. (2024). Compact Language Models via Pruning and Knowledge Distillation. \textit{arXiv:2407.14679}.
